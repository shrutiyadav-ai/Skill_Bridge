...
export default function StudentInternshipsPage() {
  ...
  const [applyingItem, setApplyingItem] = useState<InternshipItem | null>(null);
  ...
  const filteredInternships = internships.filter((item) => {
    ...
    if (selectedMode !== "ALL") {
      if (selectedMode === "REMOTE" && !item.remote) return false;
      if (selectedMode === "ONSITE" && item.remote) return false;
    }

    if (selectedStipend !== "ALL") {
      const stipend = item.stipend || 0;
      if (selectedStipend === "20K" && stipend < 20000) return false;
      if (selectedStipend === "40K" && stipend < 40000) return false;
    }

    return true;
  });

  const handleApplyClick = (item: InternshipItem) => {
  ...
  const handleModalSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setSuccessAlert(`Application submitted for ${applyingItem.title} at ${applyingItem.companyName}!`);
      ...
      <div className="space-y-6">
          ...
          <div>
            <div className="flex items-center gap-2">
              <h1 className="text-xl sm:text-2xl font-bold text-slate-900 dark:text-white">
                Student Internships & Industrial Training
              </h1>
            ...
            <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">
              Curated summer internships, apprenticeships, and live corporate training tailored to your academic background
            </p>
          </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
          <Card>
            <CardContent className="p-5">
              ...
              <p className="text-[11px] text-slate-500 dark:text-slate-400 mt-2">Active corporate openings</p>
            </CardContent>
          </Card>
          ...
          <Card className="border-blue-200 dark:border-blue-900/50 bg-blue-50/20 dark:bg-blue-950/10">
            <CardContent className="p-5">
              ...
              <div className="text-3xl font-extrabold text-navy-800 dark:text-blue-400 mt-1 font-mono">
                {internships.filter((i) => i.compatibilityScore >= 80).length}
              </div>
              <p className="text-[11px] text-slate-500 dark:text-slate-400 mt-2">Aligned with your skill vector</p>
            </CardContent>
          </Card>
          ...
          <Card>
            <CardContent className="p-5">
              ...
              <p className="text-[11px] text-slate-500 dark:text-slate-400 mt-2">Submitted this semester</p>
            </CardContent>
          </Card>
        </div>
        ...
        <div className="bg-white dark:bg-slate-900 p-4 rounded-lg border border-slate-200 dark:border-slate-800 shadow-xs space-y-3 transition-colors duration-150">
          <div className="flex flex-col sm:flex-row gap-3 items-center justify-between">
            ...
            <div className="relative w-full sm:w-80">
              ...
              <input
                type="text"
                placeholder="Search role, company, or required skill..."
                value={searchQuery}
            ...
            <div className="flex items-center gap-2 w-full sm:w-auto">
              ...
              <select
                ...
                className="input-field py-1.5 text-xs text-slate-800 dark:text-slate-200 bg-white dark:bg-slate-800 border-slate-300 dark:border-slate-700"
              >
                <option value="ALL">All Stipend Ranges</option>
        ...
        {isLoading ? (
          <div className="py-16 text-center text-slate-500 bg-white dark:bg-slate-900 rounded-lg border border-slate-200 dark:border-slate-800">
            ...
            <p className="text-xs">Loading active internships...</p>
          </div>
                  ...
                  <CardContent className="space-y-3.5 pt-0">
                    ...
                    <div className="grid grid-cols-2 gap-2 text-[11px] text-slate-600 dark:text-slate-400 p-2.5 rounded bg-slate-50 dark:bg-slate-850 border border-slate-200/80 dark:border-slate-800">
                      ...
                      <div className="flex items-center gap-1.5">
                        ...
                        <span>Mode: <strong>{item.remote ? "Remote" : item.location}</strong></span>
                      </div>
                      <div className="flex items-center gap-1.5 col-span-2 text-emerald-700 dark:text-emerald-400 font-semibold">
                        <DollarSign className="h-3.5 w-3.5 text-emerald-500 shrink-0" />
                        <span>Stipend: ₹{item.stipend?.toLocaleString("en-IN") || "35,000"} / month</span>
                      </div>
          ...
          <EmptyState
            ...
            description="Try changing your search query or stipend range."
            action={
              <Button
                size="sm"
                variant="outline"
            ...
            <div className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 max-w-lg w-full p-6 shadow-xl space-y-4">
              <div className="flex items-start justify-between">
                <div>
                  ...
                  <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">
                    {applyingItem.companyName} • Stipend: ₹{applyingItem.stipend?.toLocaleString("en-IN")}/month
                  </p>
                </div>
                <button
              ...
              <form onSubmit={handleModalSubmit} className="space-y-3.5">
                ...
                <div>
                  ...
                  <textarea
                    rows={3}
                    placeholder="Briefly state why you're a great fit for this internship..."
                    value={coverNote}
